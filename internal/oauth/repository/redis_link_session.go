...
func NewRedisLinkSessionStore(rdb *redis.Client) oauth.LinkSessionStore {
...
func (s *redisLinkSessionStore) Create(ctx context.Context, data *oauth.LinkSessionData) (string, error) {
...
func (s *redisLinkSessionStore) Consume(ctx context.Context, rawToken string) (*oauth.LinkSessionData, error) {
	hash := sha256.Sum256([]byte(rawToken))
	hashHex := hex.EncodeToString(hash[:])
	...
	if _, err := pipe.Exec(ctx); err != nil && err != redis.Nil {
		return nil, errs.ErrLinkSessionExpired
	}
	...
	var data oauth.LinkSessionData
