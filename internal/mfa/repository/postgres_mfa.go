...
func NewPostgresMFARepo(db *pgxpool.Pool) mfa.MFACredentialRepository {
	return &postgresMFARepo{db: db}
}

func (r *postgresMFARepo) Create(ctx context.Context, cred *mfa.MFACredential) error {
	const query = `
...
func (r *postgresMFARepo) FindVerifiedByUserID(ctx context.Context, userID string) (*mfa.MFACredential, error) {
	const query = `
...
func (r *postgresMFARepo) FindByUserID(ctx context.Context, userID string) (*mfa.MFACredential, error) {
	const query = `
		SELECT id, user_id, type, secret_encrypted, verified, created_at, last_used_at
		FROM   mfa_credentials
		WHERE  user_id = $1
